...
JOIN (
    ...
    FROM cats
    WHERE name = "Murzik"
) AS filter ON cats.id = filter.id;
...
# Чтобы вывести название и цену для всех анализов, которые продавались с 5 февраля 2020 года и в течение следующей недели, можно использовать следующий код:

SELECT analysis.an_name, analysis.an_price
FROM analysis
JOIN orders ON analysis.an_id = orders.ord_an
WHERE orders.ord_datetime BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', INTERVAL 7 DAY);
